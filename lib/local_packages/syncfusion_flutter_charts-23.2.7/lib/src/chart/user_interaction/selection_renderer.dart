...
import '../../common/user_interaction/selection_behavior.dart' as sf_charts;
...
class SelectionRenderer {
  ...
  void selectDataPoints(int? pointIndex, [int? seriesIndex]) {
    if (chart is SfCartesianChart) {
      if (validIndex(pointIndex, seriesIndex, chart)) {
        ...
        final sf_charts.SelectionBehaviorRenderer selectionBehaviorRenderer =
            seriesRendererDetails.selectionBehaviorRenderer;
        final sf_charts.SelectionDetails selectionDetails = sf_charts
      ...
      for (int k = 0; k < seriesRendererDetails.segments.length; k++) {
        if (seriesRendererDetails.seriesType.contains('area') == false &&
          ...
          if (currentSegmentProperties!.seriesIndex ==
              sf_charts.SelectionHelper.getRenderingDetails(
                seriesRendererDetails.selectionBehaviorRenderer!,
  ...
  int? getCartesianPointIndex(Offset position) {
    ...
    sf_charts.SelectionHelper.getRenderingDetails(
      seriesRendererDetails.selectionBehaviorRenderer,
    ).selectionRenderer!.cartesianPointIndex = nearestDataPointIndex;
  ...
  void getSelectedSeriesIndex(
    ...
    Rect? currentSegment;
    int? seriesIndex;
    sf_charts.SelectionBehaviorRenderer? selectionBehaviorRenderer;
    CartesianChartPoint<dynamic> point;
    outerLoop:
    for (
      int i = 0;
    ...
    sf_charts.SelectionHelper.getRenderingDetails(
      selectionBehaviorRenderer,
    ).selectionRenderer!.seriesIndex = seriesIndex;
  }
  ...
  void performSelection(Offset position) {
    ...
    isInteraction = true;
    int? cartesianPointIndex;
    final sf_charts.SelectionDetails selectionDetails =
        sf_charts.SelectionHelper.getRenderingDetails(
          seriesRendererDetails.selectionBehaviorRenderer,
        );
    if (seriesRendererDetails.seriesType == 'line' ||
